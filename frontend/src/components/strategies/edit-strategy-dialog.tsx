...
export function EditStrategyDialog({
  ...
  useEffect(() => {
    if (strategy) {
      ...
      setCapital(String(strategy.allocated_capital));
      setMaxPos(String(strategy.max_position_pct));
      setMaxDD(String(strategy.max_drawdown_pct));
    }
  }, [strategy]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(`Strategy "${name}" updated`, "success");
      onUpdated();
      onClose();
    } catch (err: any) {
  ...
  return (
    <Dialog open={!!strategy} onClose={onClose}>
      <DialogHeader>
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500/20 to-orange-500/20 border border-amber-500/10">
