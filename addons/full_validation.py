...
def fetch_binance(symbol, interval, start_date="2020-01-01", end_date="2026-03-19"):
...
def sig_mtf_pyramid(c, h, l, ts, d_ema200_al, st4h_dir_al, st15_line, st15_dir, rsi15, warmup=WARMUP):
    ...
    for i in range(warmup, n):
        ...
        if sig[i-1] == 0:
            ...
            if sl > 0:
                dist = (c[i]-sl)/sl; near = 0 <= dist <= 0.005
            else: near = False
...
def _worker_run_asset(args):
    ...
    try:
        ...
        # Return timestamps aligned to ec (one per bar from WARMUP to n)
        n_bars = len(c)
        ec_ts = ts[WARMUP:WARMUP+len(ec)].tolist() if len(ts) >= WARMUP+len(ec) else ts[-len(ec):].tolist()
        return sym, st, trades, ec.tolist() if isinstance(ec, np.ndarray) else ec, regime.tolist(), ec_ts
    except Exception as e:
...
def phase1_revalidation():
    ...
    asset_stats = {}
    all_trades = []
    asset_ec_data = {}  # {sym: (ec_list, ts_list)} for proper alignment
    regime_dist = {}
    ...
    for r in results:
        if r is None or r[1] is None:
            ...
            continue
        sym, st, trades, ec, regime, ec_ts = r
        asset_stats[sym] = st
        all_trades.extend(trades)
        asset_ec_data[sym] = (ec, ec_ts)
        regime_dist[sym] = {"bull": st["regime_bull_pct"], "bear": st["regime_bear_pct"], "flat": st["regime_flat_pct"]}

    # Build combined portfolio equity curve by aligning timestamps and summing
    # Collect all unique timestamps across all assets
    all_ts_set = set()
    for sym, (ec, ec_ts) in asset_ec_data.items():
        all_ts_set.update(ec_ts)
    all_ts_sorted = sorted(all_ts_set)

    # For each asset, create a mapping from timestamp to equity value
    # At each timestamp, sum the equity contribution from all assets
    portfolio_ec = np.full(len(all_ts_sorted), 0.0)
    for sym, (ec, ec_ts) in asset_ec_data.items():
        # Build ts→equity map for this asset
        ts_to_eq = {}
        for t, e in zip(ec_ts, ec):
            ts_to_eq[t] = e
        # For each portfolio timestamp, add this asset's equity (forward-fill)
        last_eq = CAPITAL  # Starting equity per asset
        for i, t in enumerate(all_ts_sorted):
            if t in ts_to_eq:
                last_eq = ts_to_eq[t]
            portfolio_ec[i] += last_eq
    ...
    # Combined portfolio stats using properly aligned equity curve
    combined = compute_stats(all_trades, portfolio_ec, CAPITAL * len(asset_ec_data))
    ...
    # Bootstrap 500 synthetic histories using 2-week block resampling
    # Group trades into 2-week windows to preserve temporal correlation
    trade_pnls = [t["pnl"] for t in all_trades]
    nt = len(trade_pnls)
    print(f"\n  [{timestamp()}] Running 500 bootstrap iterations (2-week block resampling)...")
    boot_pfs = []; boot_pnls = []

    # Group trades into 2-week blocks by exit timestamp
    TWO_WEEKS_MS = 14 * 24 * 3600 * 1000
    if all_trades:
        min_ts = min(t["exit_ts"] for t in all_trades)
        max_ts = max(t["exit_ts"] for t in all_trades)
        blocks = []
        block_start = min_ts
        while block_start < max_ts:
            block_end = block_start + TWO_WEEKS_MS
            block_trades = [t["pnl"] for t in all_trades if block_start <= t["exit_ts"] < block_end]
            if block_trades:
                blocks.append(block_trades)
            block_start = block_end

    n_blocks = len(blocks)
    for _ in range(500):
        # Resample blocks with replacement, reassemble
        sampled_blocks = [blocks[i] for i in np.random.randint(0, n_blocks, size=n_blocks)]
        sample = [pnl for block in sampled_blocks for pnl in block]
    ...
    return asset_stats, all_trades, combined, portfolio_ec, all_ts_sorted
...
def stress_test_14():
    ...
    print(f"  Metric: Dollar P&L, MDD, Liquidation count (loss > $125 margin)")
    print(f"  Optimal = highest leverage with zero liquidations")
    print(f"{'='*120}")

    leverage_levels = [1, 2, 3, 5, 7, 10, 15, 20]

    print(f"\n  {'Lev':>5} {'P&L ($)':>12} {'MDD ($)':>12} {'MDD%':>8} {'Liquidations':>14}")
    print(f"  {'─'*60}")
    ...
    for lev in leverage_levels:
        ...
        for sym in ASSETS:
            ...
            # Liquidation: loss exceeds margin of $125
            for t in trades:
                if t["pnl"] < -FIXED:
                    liquidations += 1

        total_pnl = sum(t["pnl"] for t in all_trades)
        ec_arr = np.array(all_ec) if all_ec else np.array([CAPITAL])
        pk = np.maximum.accumulate(np.maximum(ec_arr, 1.0))
        mdd_dollar = float(np.max(pk - ec_arr))
        mdd_pct = float(np.max((pk - ec_arr) / pk)) * 100

        print(f"  {lev:>4}x ${total_pnl:>11.2f} ${mdd_dollar:>11.2f} {mdd_pct:>7.1f}% {liquidations:>14}")

        # Optimal = highest leverage with zero liquidations
        if liquidations == 0:
            optimal_lev = lev

        results_by_lev[lev] = {
            "pnl": round(total_pnl, 2), "mdd_dollar": round(mdd_dollar, 2),
            "mdd_pct": round(mdd_pct, 1), "liquidations": liquidations
        }

    print(f"\n  OPTIMAL LEVERAGE: {optimal_lev}x (highest with zero liquidations)")
    ...
    ALL_RESULTS["stress_test_14"] = {
        ...
        "results_by_lev": {str(k): v for k, v in results_by_lev.items()}
    }
    save_results()
...
def stress_test_16(portfolio_ec=None):
    print(f"\n{'='*120}")
    print(f"  [{timestamp()}] STRESS TEST 16 — Drawdown Recovery Analysis")
    print(f"{'='*120}")

    if portfolio_ec is None:
        # Build portfolio equity curve by aligning and summing per-asset curves
        asset_ec_data = {}
        for sym in ASSETS:
            d = load_asset(sym)
            if d is None: continue
            combined, regime = compute_all_signals(d)
            c = d["15m"]["c"]; o = d["15m"]["o"]; ts = d["15m"]["ts"]
            trades, ec_arr, eq = backtest(c, o, combined, ts)
            ec_ts = ts[WARMUP:WARMUP+len(ec_arr)].tolist() if len(ts) >= WARMUP+len(ec_arr) else ts[-len(ec_arr):].tolist()
            asset_ec_data[sym] = (ec_arr.tolist(), ec_ts)

        all_ts_set = set()
        for sym, (ec_list, ec_ts) in asset_ec_data.items():
            all_ts_set.update(ec_ts)
        all_ts_sorted = sorted(all_ts_set)
        portfolio_ec = np.full(len(all_ts_sorted), 0.0)
        for sym, (ec_list, ec_ts) in asset_ec_data.items():
            ts_to_eq = {}
            for t, e in zip(ec_ts, ec_list):
                ts_to_eq[t] = e
            last_eq = CAPITAL
            for i, t in enumerate(all_ts_sorted):
                if t in ts_to_eq:
...
def stress_test_20():
    ...
    # Use 10x leverage for realistic simulation
    opt_lev = 10
    ...
    for sym in ASSETS:
        ...
        # Misclassification: randomly flip 3% of ACTIVE regime bars only
        # Never change FLAT bars — those are correctly flat
        combined, regime = compute_all_signals(d)
        n = len(combined)
        ...
        for i in range(n):
            if flip_mask[i]:
                if regime[i] == 1: regime_noisy[i] = np.random.choice([2, 0])  # Bull → Bear or Flat
                elif regime[i] == 2: regime_noisy[i] = np.random.choice([1, 0])  # Bear → Bull or Flat
                # regime[i] == 0 (FLAT): do NOT flip — correctly flat
...
def phase4_final():
    ...
    opt_lev = ALL_RESULTS.get("stress_test_14", {}).get("optimal_lev", 10)
...
def main():
    ...
    # ST16 gets the corrected portfolio equity curve
    tests_phase3 = [
        ("Stress Test 16", lambda: stress_test_16(portfolio_ec=portfolio_ec)),
